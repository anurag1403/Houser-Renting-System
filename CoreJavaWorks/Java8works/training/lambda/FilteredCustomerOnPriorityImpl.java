...
public class FilteredCustomerOnPriorityImpl {
	//before java 8
	...
	public static List<PriorityCustomer> withSalaryMoreThanInJava8WithFilter(List<Customer> customers) {
		return customers.stream().filter(temp -> 
			temp.getSalary() > 5000
		).map(temp -> {
			return new PriorityCustomer(temp.getCustId(),temp.getCustName(), true);
		}).collect(Collectors.toList());
	...
	public static void main(String[] args) {
		List<Customer> customers = Arrays.asList(new Customer(101, "Sandeep", 8000, "Developer"),
				new Customer(102, "Harry", 2000, "Accountant"),
				new Customer(103, "Sam", 10000, "Software Engineer"),
				new Customer(104, "Ron", 3000, "Plumber")				
				);
